...
#     with Pool(processes=5) as pool:
    ...
    ga_instance = GA_super(codon_to_int,gene_space,gene_space_int,population_size,tissue)
...
#         super().__init__(fitness_func=self.fitness_func,*args,**kwargs)
    ...
    def __init__(self,codon_to_int,gene_space,gene_space_int,population_size,tissue):
        # self.pool = Pool(processes=2)    
        self.all_sols = {}
...
#                         bai_weight_dict=self.bai_weight_dict)
    ...
    @staticmethod
    def get_defaults(gene_space,gene_space_int,population_size):
        variables={
                'num_generations':5,
    ...
    def process_wrapper(self,pop_fitness,inds):
        for ind in inds:
            selected_pop = self.population[ind]
            pop_fitness[ind] = self.fitness_wrapper(selected_pop)

    def assign_tasks(self,threads,population):
        n = int(np.ceil(len(population)/threads))
        lst = range(len(population))
        splits = [lst[i::n] for i in range(0, threads)]
        return(splits)
    ...
    def cal_pop_fitness(self):
        ...
        with Manager() as manager:
            ...
            init_list = [0] * len(population)

            pop_fitness = manager.list(init_list)
            splits = self.assign_tasks(threads,population)
            ps = [0]*threads
           
            for i in range(threads):
                # process_wrapper(square,split)
                inds = splits[i]
                ps[i] = Process(target=self.process_wrapper, args=(pop_fitness,inds))
                ps[i].start()
        ...
        # codon_to_int = self.codon_to_int
        # cai_weight_dict = self.cai_weight_dict
        # bai_weight_dict = self.bai_weight_dict
        
#         fitpart=partial(self.fitness_func, sol_ind=0,
#                         codon_to_int=codon_to_int,
#                         cai_weight_dict=cai_weight_dict,
#                         bai_weight_dict=bai_weight_dict
#                        )
        # fitpart = self.fitness_wrapper
        # pop_fitness = self.pool.map(fitpart, self.population)
        return pop_fitness
        
    
    # ftiness function
    @staticmethod
    def fitness_func(solution,sol_ind):
